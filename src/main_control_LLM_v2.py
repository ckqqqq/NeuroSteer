...
from data_preprocess import (
    load_and_prepare_triple_dataset,
    load_and_prepare_COT_dataset,
    load_and_prepare_debate_triple_dataset,
    load_and_prepare_polite_dataset,
)
...
parser.add_argument(
    ...
    type=str,
    default="sentiment",
    choices=["sentiment", "cot", "polite", "toxicity", "debate"],
    help="Task type: sentiment, cot, polite",
)
...
parser.add_argument(
    "--topk_mean", type=int, default=100, help="Top K mean selection"
)  # 目前没有用了
parser.add_argument(
    "--topk_cnt", type=int, default=100, help="Top K count selection"
)  # 主要用这个
parser.add_argument("--batch_size", type=int, default=32, help="Batch size")
parser.add_argument(
    "--source", type=str, default="pos", help="Source class"
)  # 赞同、积极情感、礼貌、COT、无毒性
parser.add_argument(
    "--target", type=str, default="neg", help="Target class"
)  # 不赞同、消极情感、不礼貌、直接推理、无毒性
parser.add_argument(
    "--prompt_source",
    type=str,
    default="",
...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="用于评估的prompt的条数，默认-1为全部",
)
parser.add_argument(
    "--mean_type",
    type=str,
    default="dif_mean",
    choices=["dif_mean", "tar_mean"],
    help="Mean type",
)
parser.add_argument(
    "--steer_type",
    type=str,
    default="last",
    choices=["all", "last", "last2", "gaussian"],
    help="Steer type",
)
parser.add_argument(
...
parser.add_argument(
    "--dataset_path",
    type=str,
    default="/home/ckqsudo/code2024/0dataset/baseline-acl/data/sentiment/sst5",
    help="Dataset path",
)
parser.add_argument(
    "--prompt_path",
    type=str,
    default="/home/ckqsudo/code2024/0dataset/baseline-acl/data/prompts/sentiment_prompts-10k",
    help="Prompt path",
)
parser.add_argument("--env_path", type=str, default=".env", help="Environment path")
# 解码参数
parser.add_argument(
    "--temperature", type=float, default=0.9, help="Sampling temperature for generation"
)
parser.add_argument(
    "--top_p", type=float, default=0.3, help="Top-p (nucleus) sampling parameter"
)
parser.add_argument(
    "--freq_penalty", type=float, default=1.0, help="Frequency penalty for generation"
)
parser.add_argument(
    "--example_prompt", type=str, default="I feel", help='Example prompt eg "I FEEL"'
)
# 垃圾args，用布尔会有奇怪的东西
parser.add_argument(
    "--debug",
    ...
    default=0,
    choices=[0, 1],
    help="Debug flag: 0 for False, 1 for True",
)
parser.add_argument(
    "--save_no_steer",
    type=int,
    default=0,
    choices=[0, 1],
    help="是否需要比较GPT原先生成的结果",
)
parser.add_argument(
    "--is_norm_delta_matrix",
    type=int,
    default=0,
    choices=[0, 1],
    help="是否需要对delta矩阵进行归一化（如果归一化算出来的alpha理论上更体现（单位）数值，但是实际实验结果表明没多大影响，这玩意和alpha挂钩哦，实验的时候确保统一）",
)
parser.add_argument(
    "--use_cache", type=int, default=0, choices=[0, 1], help="是否需要保存steer_info?"
)
...
parser.add_argument(
    ...
    default=2,
    choices=[0, 1, 2, 3, 4, 5],
    help="重复生成每个prompt的次数",
)
parser.add_argument(
    "--gen_batch_size",
    type=int,
    default=1,
    help="批量生成的时候生成batch大小，基于显卡调整",
)
# 注意：不去使用则覆写！
...
assert isinstance(EXAMPLE_PROMPT_LIST, list) and isinstance(
    EXAMPLE_PROMPT_LIST[0], str
), "EXAMPLE_PROMPT_LIST必须是list[str]"
...
CACHE_DIR = os.path.join(
    args.output_dir,
    f"{args.LLM}_{TASK}_layer_{args.layer}_datasize_{DATA_SIZE}_batchsize{args.batch_size}_topK_{args.topk_cnt}",
)
OUTPUT_DIR = os.path.join(
    CACHE_DIR,
    f"alpha_{args.alpha}_from_{args.source}_to_{args.target}_prompt_{args.prompt_source}_mean_{args.mean_type}_steertype_{args.steer_type}_device_{args.device}",
)
...
if TASK == "sentiment":
    neg_train_set, pos_train_set, neu_train_set, val_set, test_set = (
        load_and_prepare_triple_dataset(
    ...
    neg_train_set, pos_train_set, _, _, _ = load_and_prepare_toxicity_dataset(
        args.dataset_path, task=TASK, seed=args.seed
    )
    print(neg_train_set)
elif "polite" == TASK:
    logging.info("polite" * 10)
    pos_train_set, neg_train_set, neu_train_set, polite_test_set = (
        load_and_prepare_polite_dataset(args.dataset_path, args.seed)
    )
elif "debate" == TASK:
    logging.info("debate" * 10)
    pos_train_set, neg_train_set, val_set, test_set = (
        load_and_prepare_debate_triple_dataset(args.dataset_path, args.seed)
    )
else:
    raise ValueError("No Supported")


####################################################################### LOAD SAE+GPT+TOKENIZER
logging.info(f"Loading Model Loading SAE for layer {args.layer} {args.LLM}")
from model_preprocess import load_models

model, sae, tokenizer = load_models(LLM=args.LLM, layer=args.layer, device=args.device)
logging.info(f"model architecture for {args.LLM} {model} {tokenizer}")
########################################################################## Neuron Selection (GPT -> SAE)
...
def SAE_encoding(
    ...
    lat_freq, lat_val_sum = (
        torch.zeros(SAE_LATENT_SIZE).to("cpu"),
        torch.zeros(SAE_LATENT_SIZE).to("cpu"),
    )  # 避免OOM
    # 使用 tqdm 显示进度条
    with torch.no_grad():
        for i in tqdm(range(0, len(texts), batch_size), desc="Processing batches"):
            ...
            try:
                sv_logits, cache = model.run_with_cache(
                    batch_texts, prepend_bos=True, device=args.device
                )
    ...
    lat_val_mean = torch.where(
        lat_freq != 0, lat_val_sum / lat_freq, torch.tensor(0.0, device="cpu")
    )
    logging.info(f"Total non-zero element shape: {lat_freq.shape}")

    assert lat_freq.shape[0] == lat_val_mean.shape[0] == sae.W_dec.shape[0], (
        "sae latent dimension mismatch"
    )
    return {
        "latent_frequency": lat_freq.to(device),
        "latent_value_mean": lat_val_mean.to(device),
    }
...
def neuron_selection(texts: list):
    ...
    return {
        "latent_value_mean": lat_info["latent_value_mean"],
        "latent_frequency": lat_info["latent_frequency"],
    }
...
def compute_neuron_info():
    ...
    if TASK == "polite" or TASK == "sentiment":
        ...
        if "pos" in steer_polar_list:
            ...
            neuron_info["pos"] = neuron_selection(pos_train_set["text"][:DATA_SIZE])
        if "neg" in steer_polar_list:
            logging.info(f"negative")
            neuron_info["neg"] = neuron_selection(neg_train_set["text"][:DATA_SIZE])
        if "neu" in steer_polar_list:
            logging.info(f"neutral")
            neuron_info["neu"] = neuron_selection(neu_train_set["text"][:DATA_SIZE])
    else:
        # 不包含neu的处理
        if DATA_SIZE == "ALL":
            DATA_SIZE = min([steer_lens[steer] for steer in steer_polar_list])
...
# 缓存到文件里面######################################################### Save Neuron Info into files for (demo / linear combination /exploring tasks)
from utils import load_or_cache_neuron_info

neuron_info = load_or_cache_neuron_info(
    cache_dir=CACHE_DIR,
    use_cache=bool(args.use_cache==1),
    cache_filename=f"{TASK}_neuron_info_cache_of_{args.LLM}_l{args.layer}.pkl",
    compute_func=compute_neuron_info,
)
...
assert TARGET in neuron_info.keys() and SOURCE in neuron_info.keys(), (
    str(neuron_info.keys()) + "请检查source和target是否正确"
)
assert not torch.all(
    (
        neuron_info[TARGET]["latent_value_mean"]
        - neuron_info[SOURCE]["latent_value_mean"]
    )
    == 0
), "dataset reading error, check key (neg,pos)请检查很可能neg pos写混了"
assert torch.all(neuron_info[TARGET]["latent_value_mean"] >= 0), (
    "所有SAE的激活需要大于d等于0（maybe）"
)

logging.info(f"转向方向 dif_{TARGET}-{SOURCE}_relu")


############################################################### Denoising （[CORE] get topk_freqency_diffence_neurons）

from denoising import denosing

neuron_info[f"dif_{TARGET}-{SOURCE}_relu"] = denosing(
    neuron_info=neuron_info, target=TARGET, source=SOURCE, top_k=args.topk_cnt
)# CORE FUNCTION


logging.info(
    "sae cfg.hook_name (resadd position): " + str(sae.cfg.hook_name)
)  # 挂载名称


assert args.mean_type in ("dif_mean", "tar_mean"), "Update SAE_decoding code"

if args.mean_type == "dif_mean":
    latent_value_mean = neuron_info[f"dif_{TARGET}-{SOURCE}_relu"]["latent_value_mean"]
    logging.info("使用差值的激活值均值")
elif args.mean_type == "tar_mean":
    latent_value_mean = neuron_info[TARGET]["latent_value_mean"]
    logging.info("使用目标的激活值均值")

topk_neurons_indices=neuron_info[f"dif_{TARGET}-{SOURCE}_relu"]["topk_freq_dif"]

################################################################ get Delta h (SAE -> GPT)
# 人工激活这些神经元，将其映射回GPT的残差流中
from intervention_generation import SAE_decoding

delta_h = SAE_decoding(
    sae,
    target_neurons_indices=topk_neurons_indices,
    latent_value_mean=latent_value_mean,
    is_norm=args.is_norm_delta_matrix,
)
...
########################################################### apply_intervention (add delta h into gpt residual)
...
logging.info(
    f"干预方向{SOURCE}->{TARGET},礼貌任务下，neg=impolite，情感任务下 pos=积极情感"
)
...
# * 如果对[0:-1]的区间进行干预，效果异常优秀，生成比较连贯，但是如果对[-1:length]的区间进行干预，效果就很差，生成的词语很零碎，干预还是需要竟可能连贯
# # 下面进行的是扭转实验，使用prompt对模型进行诱导，再进行转向

# %%
################################################################### EVAL ON FULL/ Larger DATASET
...
def eval_on_full_data():
    ...
    if TASK == "sentiment":
        ...
        prompts = load_and_prepare_sentiment_prompts(
            prompt_path=args.prompt_path, task=TASK
        )
    elif TASK == "polite":
        logging.info(
            "In Domain: Calculate at A train dataset, Evaluate at A test dataset"
        )
        ...
        prompts = load_and_prepare_toxicity_prompts(
            prompt_path=args.prompt_path, task=TASK
        )
    elif TASK == "debate":
        logging.info("Out of Domain: Calculate at A dataset, Evaluate at B dataset")
        ...
        prompts = load_and_prepare_debate_prompts(
            prompt_path=args.prompt_path, task=TASK
        )
    else:
        raise NotImplementedError("No Supported Task")
    print(prompts)
    assert "neg" in prompts, (
        "prompt steer source (pos/neg) not in prompts, please check the data_preprocess section"
    )
    ...
    if args.prompt_data_size != -1:  # 用于小批量网格搜索
        prompts = prompts.select(
            range(args.prompt_data_size)
        )  # Correct slicing for datase
        logging.warning("截取prompt_datasize" + str(len(prompts)))

    # 新增批次大小参数（假设从args获取）

    with jsonlines.open(
        os.path.join(OUTPUT_DIR, "no_steer_gen_res.jsonl"), mode="w"
    ) as no_steer_f:
        with jsonlines.open(
            os.path.join(OUTPUT_DIR, "steer_gen_res.jsonl"), mode="w"
        ) as steer_f:
            # 分批次处理prompts
            for batch_idx in tqdm(range(0, len(prompts), GEN_BATCH_SIZE)):
                # batch = prompts[batch_idx:batch_idx+GEN_BATCH_SIZE]
                batch = prompts.select(
                    range(batch_idx, min(batch_idx + GEN_BATCH_SIZE, len(prompts)))
                )
                ...
                if SAVE_NO_STEER == 1:
                    ...
                    for i, item in enumerate(batch):
                        ...
                        no_steer_item["generations"] = [
                            {"text": text} for text in no_steer_gen_texts_batch[i]
                        ]
                ...
                for i, item in enumerate(batch):  # 遍历每个batch中的元素
                    ...
                    steer_item["generations"] = [
                        {"text": text} for text in steered_texts_batch[i]
                    ]
                    global_idx = batch_idx + i
                    if global_idx % 20 == 0:
                        logging.info(
                            f"{TASK}: from {SOURCE} to {TARGET} prompt_set: {SOURCE}"
                        )
