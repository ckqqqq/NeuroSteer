...
from activation_engineering_utils import train_probes_with_attention_activations,get_all_head_delta_matrices_cot2judge,get_head_vector_info_list_dict,inference_with_activation_editing
import torch
import os
...
def main():
    ...
    parser.add_argument('--model', type=str,choices=list(model_config.keys()), default='qwen2.5-7')
    ...
    attention_output_activations_cot,mlp_output_activations,QA_info=load_activations(os.path.join(args.project_root_path,f"/home/ckqsudo/code2024/CKQ_ACL2024/Control_Infer/activations_data/test/COT_prompt_test_qwen2.5-3_truthful_qa_COT_activations"))
    attention_output_activations_judge,_,_=load_activations(os.path.join(args.project_root_path,f"activations_data/test/{args.model}_{args.dataset}_{args.task}_activations"))
    ...
    for i in range(args.num_fold):
        ...
        probes,acc_probes=train_probes_with_attention_activations(train_data_idxs=train_data_idxs, val_data_idxs=val_data_idxs, attention_output_activations=attention_output_activations_judge,QA_info=QA_info,num_layer=LAYER,num_head=HEAD,seed=args.seed)    
        ...
        if args.method in ["com","SVD"]:
            ...
            all_head_delta_matrices=get_all_head_delta_matrices_cot2judge(all_data_indices=all_data_indices,attention_output_activations_cot=attention_output_activations_cot,attention_output_activations_judge=attention_output_activations_judge, QA_info=QA_info,num_layer=LAYER,num_head=HEAD)
            if args.method=="com":
                # 基于detal矩阵计算变化的方向和幅度
                head_vector_info_list_dict=get_head_vector_info_list_dict(all_head_delta_matrices=all_head_delta_matrices,top_head_indices=top_head_indices,num_head=HEAD,num_layer=LAYER,all_data_indices=all_data_indices,attention_output_activations=attention_output_activations_judge)
            else:
