...
output_dir=os.path.join(output_dir,f"alpha_{args.alpha}_from_{args.source}_to_{args.target}_datasize_{args.data_size}_layer_{args.layer}_mean_{args.mean_type}_steertype_{args.steer_type}_device_{args.device}")
...
def get_activation_by_steer(texts:list):
    ...
    act_cnt=lat_info["latent_frequency"]
    nz_mean=lat_info["latent_value_mean"]
    # # 计算第二个维度的最大值
...
with jsonlines.open(os.path.join(output_dir,"no_steer_gen_res.jsonl"), mode='a') as nt_file:
    with jsonlines.open(os.path.join(output_dir,"steer_gen_res.jsonl"), mode='a') as t_file: 
