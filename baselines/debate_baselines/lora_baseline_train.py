...
lora_config = LoraConfig(
    r=8192,                  # LoRA rank, 控制低秩矩阵的维度
    lora_alpha=128,        # LoRA alpha，控制低秩矩阵与原模型权重的融合比例
    target_modules=["c_attn", "c_proj"],  # 目标模块指定要进行LoRA微调的模块
...
train_dataset = train_dataset.map(pad_data, batched=True)
validation_dataset = validation_dataset.map(pad_data, batched=True)

print(train_dataset[0])
train_dataset = train_dataset.remove_columns(['metadata', 'text', 'id'])
validation_dataset = validation_dataset.remove_columns(['metadata', 'text', 'id'])
